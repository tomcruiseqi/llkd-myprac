...
MODULE_DESCRIPTION("LKP-2 book:user_kernel_comm/procfs_simple_intf: simple "
                   "procfs interfacing demo");
...
static int proc_show_drvctx(struct seq_file *seq, void *v) {
    ...
    seq_printf(seq,
               ...
               "oursecret:%s\n",
               OURMODNAME, gdrvctx->tx, gdrvctx->rx, gdrvctx->err,
               gdrvctx->myword, gdrvctx->power, gdrvctx->config1,
               gdrvctx->config2, gdrvctx->config3, gdrvctx->oursecret);
    mutex_unlock(&mtx);
    return 0;
}
...
static int proc_show_pgoff(struct seq_file *seq, void *v) {
    seq_printf(seq, "%s:PAGE_OFFSET:0x%px:data%s\n", OURMODNAME,
               (void *)PAGE_OFFSET, (char *)seq->private);
    return 0;
}
